      ...
      <li class="nav-item dropdown mt-1">
		...
		<main class="w-100">
            <div class="d-flex justify-content-center">
                <div class='text-center p-5 rounded bg-light-subtle shadow'>
                    <h1 class='h1 mb-3 fw-bold text-center text-success'>Results</h1>
                    <hr class="col-10 m-auto mb-5 pb-2 border-3">
                            <?php
                                    ...
                                    if ($search_word!=='') {
                                        ...
                                        $tables_result = mysqli_query($connection, $show_tables);
                                        ...
                                        if ($search_word !== '') {
                                            ...
                                            while ($table_row = mysqli_fetch_array($tables_result, MYSQLI_NUM)) {
                                                //pick all the rows of the tables in the variable $table_name. 
                                                ...
                                                //create an array that contains all the word searching in the columns that mySQL have to do.
                                                $conditions = [];

                                                while ($column_row = mysqli_fetch_array($columns_result, MYSQLI_ASSOC)) {
                                                    $column_name = $column_row['Field'];
                                                    $conditions[] = "$column_name LIKE '%$search_word%'";
                                                }
                                                
                                                // Construct the search query for the current table
                                                if (!empty($conditions)) {
                                                    //add the OR logic operator in the conditions variable 
                                                        ...
                                                        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                                            foreach ($row as $column => $value) {
                                                              if (stripos($value, $search_word) !== false) {
                                                                //replace the searched word with the styled search_word in the value string.
                                                                //str_ireplace is case insensitive
                                                                $highlighted_value = str_ireplace($search_word, "<span class='bg-success rounded'>$search_word</span>", $value);
                                                                $search_results .= "<h4 class='h4 fw-bold p-2 text-start'> - $highlighted_value</h4>";
                                                              }
                                                            }
                                                        }
                                                    }
                                            ...
                                            //if $found_results is true, then print the variable $search_results with all his data, else print no results.
                                            if ($found_results) {
                                                echo $search_results;
                                            } else {
                                                echo "<h4 class='h4 fw-bold p-2'>0 results retrieved</h4>";
                                            }
                                        }
